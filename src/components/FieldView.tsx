  ...
  return (
    <div className="min-h-screen bg-black text-[#e6edf3] flex flex-col max-w-md mx-auto border-x border-[var(--line)]">
      <header className="p-4 border-b border-[var(--line)] bg-[#0b0f14] sticky top-0 z-10 flex items-center justify-between">
        <div>
          <h1 className="font-bold text-lg text-[#34d399]">Field Operations</h1>
          <div className="text-xs text-[var(--muted)]">{me.name} · {tasks.length} Assigned Tasks</div>
        </div>
        <button onClick={async () => { await api("/api/auth/logout", { method: "POST" }); setMe(null); }} className="p-2 text-xs text-[var(--muted)]">Log out</button>
      </header>
      ...
      <div className="p-4 space-y-4 overflow-y-auto flex-1 pb-24">
        {tasks.length === 0 ? (
          ...
          tasks.sort((a, b) => b.created_at - a.created_at).map(t => (
            <div key={t.id} onClick={() => setSelectedTask(t)} className="bg-[#121820] border border-[var(--line)] rounded-xl p-4 active:scale-[0.98] transition-transform cursor-pointer shadow-lg shadow-black/50">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold font-mono text-[#94a3b8]">{t.entity_id}</span>
                <span className="pill text-[10px]" style={{ background: "rgba(0,0,0,0.5)", border: `1px solid ${FALLBACK_TASK_COLOR[t.state] ?? "#64748b"}`, color: FALLBACK_TASK_COLOR[t.state] ?? "#94a3b8" }}>
                  {t.state}
                </span>
              </div>
              <div className="text-sm font-semibold mb-1">Intervention: {t.intervention_class_id}</div>
              <div className="text-xs text-[var(--muted)]">Dispatched {fmtAgo(t.created_at)}</div>
              {t.sla_deadline && <div className="text-xs text-[#fbbf24] mt-1">SLA Deadline: {new Date(t.sla_deadline).toLocaleTimeString()}</div>}
            </div>
          ))
        )}
      </div>
  ...
  return (
    <div className="min-h-screen bg-black text-[#e6edf3] flex flex-col max-w-md mx-auto border-x border-[var(--line)]">
      <header className="p-4 border-b border-[var(--line)] bg-[#0b0f14] sticky top-0 z-10 flex items-center gap-3">
        <button onClick={onBack} className="p-2 -ml-2 text-[var(--muted)] active:text-white">← Back</button>
      ...
      <div className="p-4 flex-1 overflow-y-auto pb-24">
        ...
        <div className="bg-[#121820] border border-[var(--line)] rounded-xl p-4 mb-4">
          <div className="text-xs text-[var(--muted)] mb-1 uppercase tracking-wider">Current Status</div>
          <div className="text-lg font-bold" style={{ color: FALLBACK_TASK_COLOR[task.state] ?? "#fff" }}>{task.state}</div>
          <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
            <div>
              <div className="text-[var(--muted)] text-xs">Action</div>
              <div className="font-semibold">{task.intervention_class_id}</div>
            </div>
            <div>
              <div className="text-[var(--muted)] text-xs">SLA Status</div>
        ...
        {!isFinished && !needsProof && (
          <div className="space-y-3 mt-8">
            <h3 className="text-sm font-bold text-[var(--muted)] uppercase tracking-wider mb-2">Execute Action</h3>
            ...
            {task.state === "ACCEPTED" && (
              <button disabled={busy} onClick={() => handleTransition("IN_PROGRESS")} className="w-full py-4 rounded-xl font-bold bg-[#eab308] text-black active:scale-95 transition-transform text-lg shadow-[0_0_20px_rgba(234,179,8,0.3)]">
                {busy ? "Updating..." : "START INTERVENTION"}
              </button>
            )}
        ...
        {isFinished && (
          <div className="mt-8 p-4 bg-[#064e3b]/20 border border-[#064e3b] rounded-xl text-center">
            ...
            <div className="font-bold text-[#34d399]">Workflow Complete</div>
            <p className="text-sm text-[var(--muted)] mt-1">This task is now with supervisors for verification or already resolved.</p>
          </div>
        )}
      </div>
    </div>
...
function EvidenceCaptureForm({ task, onRefresh }: { task: TaskView; onRefresh: () => void }) {
  ...
  return (
    <div className="mt-8">
      <h3 className="text-sm font-bold text-[var(--muted)] uppercase tracking-wider mb-4">Capture Evidence</h3>
      {error && <div className="p-3 mb-4 rounded-lg bg-red-950/50 border border-red-900 text-red-300 text-sm">{error}</div>}
      ...
      <div className="space-y-4">
        <div className="p-6 bg-[#1a232f] border border-[#2d3b4a] rounded-xl flex flex-col items-center justify-center gap-2 text-[var(--muted)] cursor-not-allowed">
          ...
          <span className="text-sm">Camera API (Simulated)</span>
          <span className="text-[10px] text-center max-w-[200px]">1 photo reference will be attached to this proof.</span>
        </div>
        
        <div>
          <label className="text-xs text-[var(--muted)] block mb-1">Field Notes</label>
          <textarea 
